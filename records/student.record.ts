...
export class StudentRecord implements StudentEntity {
    ...
    async insert(): Promise<void> {
        ...
        const [results] = (await pool.execute(
            'INSERT INTO `student` (`id`, `email`, `courseCompletion`, `courseEngagement`, `projectDegree`, `teamProjectDegree`, `bonusProjectUrls`) VALUES (:id, ,:email :courseCompletion, :courseEngagement, :projectDegree, :teamProjectDegree, :bonusProjectUrls)',
            {
                id: this.id,
                email: this.email,
    ...
    async update():Promise<void> {
        await pool.execute("UPDATE `student` SET `tel` = :tel, `lastName` = :lastName, `firstName` = :firstName, `githubUserName` = :githubUserName, `portfolioUrls` = :portfolioUrls, `projectUrls` = :projectUrls, `bio` = :bio, `expectedTypeWork` = :expectedTypeWork, `targetWorkCity` = :targetWorkCity, `expectedContractType` = :expectedContractType, `expectedSalary` = :expectedSalary, `canTakeApprenticeship` = :canTakeApprenticeship, `monthsOfCommercialExp` = :monthsOfCommercialExp, `education` = :education, `workExperience` = :workExperience, `courses` = :courses, `status` = :status, `user_id` = :user_id, `hr_id` = :hr_id WHERE `id` = :id", {
            id: this.id,
            tel: this.tel,
